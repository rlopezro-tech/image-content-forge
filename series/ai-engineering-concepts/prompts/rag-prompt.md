...
Primary request: Create a premium, modern, clean engineering reference card about the AI engineering concept "Retrieval-Augmented Generation". Use a soft light-gray editorial canvas with white content bands, thin cool-gray borders, soft layered shadows, a bright white header, a gradient blue/purple concept icon tile, a compact blue type badge, a smaller dark navy main title, overview copy that uses the full available text area, and larger readable body text. Use one electric blue accent plus restrained teal, purple, and amber functional accents. Make the central flow diagram the visual anchor with numbered nodes, crisp connectors, and varied 2D visual states. Use recognizable standard technical glyphs with consistent stroke weight.

Canvas: 1080 x 1350 px.

Visible copy:

- Header: `RAG` and `MENTAL MODEL`.
- Overview: `Retrieval-Augmented Generation connects a model to external knowledge. The system retrieves relevant context first, then the model generates an answer grounded in those sources.`
- How it works: `User question` -> `Retrieve context` -> `Rank sources` -> `Generate answer` -> `Cited response`.
- Implementation:
  - `Chunk, embed, and index trusted source content.`
  - `Retrieve top matches, then filter or rerank.`
  - `Ground the answer with source snippets or citations.`
- Tools / frameworks:
  - `Retrieval: vector DBs, hybrid search, rerankers.`
